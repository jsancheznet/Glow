...
void R_Init(renderer *Renderer, i32 DrawableWidth, i32 DrawableHeight)
{
    *Renderer = {};
    Renderer->Exposure = Exposure__;
    ...
    Renderer->HardwareVendor = glGetString(GL_VENDOR);
    Renderer->HardwareModel = glGetString(GL_RENDERER);
    Renderer->OpenGLVersion = glGetString(GL_VERSION);
    Renderer->GLSLVersion = glGetString(GL_SHADING_LANGUAGE_VERSION);

    { // SUBSECTION: Compile shaders
        Renderer->BlurShader = R_CreateShader("shaders/blur.glsl");
        glUseProgram(Renderer->BlurShader);
        R_SetUniform(Renderer->BlurShader, "Image", 0);

        Renderer->BloomShader = R_CreateShader("shaders/bloom_final.glsl");
        glUseProgram(Renderer->BloomShader);
        R_SetUniform(Renderer->BloomShader, "Scene", 0);
        R_SetUniform(Renderer->BloomShader, "BloomBlur", 1);

        Renderer->CubeShader = R_CreateShader("shaders/cube_shader.glsl"); // IMPORTANT: This shader also outputs to the brightness texture

        Renderer->HdrShader = R_CreateShader("shaders/HDR.glsl");
        R_SetUniform(Renderer->HdrShader, "HDRBuffer", 0);
    ...
    { // SECTION: HDR+Bloom setup
        ...
        glBindTexture(GL_TEXTURE_2D, Renderer->ColorBuffer);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, DrawableWidth, DrawableHeight, 0, GL_RGBA, GL_FLOAT, NULL);
        ...
        glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, DrawableWidth, DrawableHeight);
        ...
        { // SUBSECTION: PingPong Framebuffers creation
            ...
            for (u32 i = 0; i < 2; i++)
            {
                ...
                glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, DrawableWidth, DrawableHeight, 0, GL_RGB, GL_FLOAT, NULL);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
